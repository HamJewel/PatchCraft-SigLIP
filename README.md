...
- **PatchCraft-SigLIP**模型的**SigLIP**模型配置信息位于`./src/Configs`目录中，格式为json。
- **PatchCraft-SigLIP**模型权重位于`./src/Configs/best.pt`(需下载)。
...
本项目使用**PatchCraft-SigLIP**模型融合，代码存放于`./src/Model`。**PatchCraft**模型着重关注图像的纹理特征，**SigLIP**模型着重理解图像的语义信息，通过融合纹理和语义特征，进一步提高模型的预测精度和泛化能力。
...
**PatchCraft-SigLIP**模型融合了图像的纹理特征和语义特征，并且保留了多模态模型对图像的语言描述的理解能力。
